...
internal class UserAndLimitationManagementLogic : IUserAndLimitationManagementLogic
{
    ...
    private AdaptorUser GetOrRegisterLexisCredentials(UserInfoExtendedModel lexisUser)
    {
        ...
        lock (_lockCreateUserObj)
        {
            ...
            var user = _unitOfWork.AdaptorUserRepository.GetByName(lexisUser.UserName);
            if (user is null)
            {
            ...
            foreach (var lexisProject in lexisProjects)
            {
                ...
                hasUserGroup = true;
                _log.Info(
                    $"LEXIS AAI: User \"{user.Username}\" for Project Short Name \"{lexisProject.ProjectShortName}\" was added to groups: \"{string.Join(',', groupsWithProject.Select(s => s.Name))}\"");
                _unitOfWork.Save();
            }
    ...
    private static void UpdateRoleForUserAndGroup(AdaptorUser user, AdaptorUserGroup group, AdaptorUserRole userRole)
    {
        ...
        {
            if (adaptorUserWithGroupRole.AdaptorUserRole == userRole) adaptorUserWithGroupRole.IsDeleted = false;
