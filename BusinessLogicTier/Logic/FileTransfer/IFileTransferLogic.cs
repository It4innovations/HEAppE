...
{
    Task RemoveJobsTemporaryFileTransferKeysAsync();
    ...
    Task EndFileTransferAsync(long submittedJobInfoId, string publicKey, AdaptorUser loggedUser);

    Task<IList<JobFileContent>> DownloadPartsOfJobFilesFromClusterAsync(long submittedJobInfoId, TaskFileOffset[] taskFileOffsets,
        AdaptorUser loggedUser);

    Task<IList<SynchronizedJobFiles>> SynchronizeAllUnfinishedJobFilesAsync();
    Task<ICollection<FileInformation>> ListChangedFilesForJobAsync(long submittedJobInfoId, AdaptorUser loggedUser);
    Task<byte[]> DownloadFileFromClusterAsync(long submittedJobInfoId, string relativeFilePath, AdaptorUser loggedUser);
