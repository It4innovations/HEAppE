...
public abstract class UnitOfWorkFactory
{
    ...
    private static readonly Dictionary<UnitOfWorkType, UnitOfWorkFactory> _factoryInstances =
        new(Enum.GetValues(typeof(UnitOfWorkType)).Length);
    ...
    public static UnitOfWorkFactory GetUnitOfWorkFactory(UnitOfWorkType type)
    {
        ...
        {
            if (!_factoryInstances.ContainsKey(type)) _factoryInstances.Add(type, CreateUnitOfWorkFactory(type));
        }
    ...
    private static UnitOfWorkFactory CreateUnitOfWorkFactory(UnitOfWorkType type)
    {
        ...
        throw new ArgumentException(
            $"Unit of work factory for type {type} is not implemented. Check the switch in HaaSMiddleware.DataAccessLayer.Factory.UnitOfWork.AbstractUnitOfWorkFactory.CreateUnitOfWorkFactory(UnitOfWorkType type) method.");
    }
