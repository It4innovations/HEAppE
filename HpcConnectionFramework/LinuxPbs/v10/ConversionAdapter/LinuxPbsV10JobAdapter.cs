...
namespace HEAppE.HpcConnectionFramework.LinuxPbs.v10.ConversionAdapter {
	public class LinuxPbsV10JobAdapter : ISchedulerJobAdapter {
		#region Constructors
		public LinuxPbsV10JobAdapter(object jobSource) {
			this.jobSource = (string) jobSource;
			qstatInfo = LinuxPbsConversionUtils.ReadQstatResultFromJobSource(this.jobSource);
		}
		#endregion
		...
		public string Id {
			get {
				...
				if (qstatInfo.TryGetValue(LinuxPbsJobInfoAttributes.JOB_ID, out result))
					return LinuxPbsConversionUtils.GetJobIdFromJobCode(result);
				return "";
			}
		}
		...
		public string AccountingString {
			get {
				string result;
				if (qstatInfo.TryGetValue(LinuxPbsJobInfoAttributes.ACCOUNT_NAME, out result))
					return result;
				return string.Empty;
			}
			set {
		...
		public DateTime CreateTime {
			get {
				string result;
				if (qstatInfo.TryGetValue(LinuxPbsJobInfoAttributes.CTIME, out result))
		...
		public DateTime? SubmitTime {
			get {
				string result;
				if (qstatInfo.TryGetValue(LinuxPbsJobInfoAttributes.ETIME, out result))
					return LinuxPbsConversionUtils.ConvertQstatDateStringToDateTime(result);
				return null;
			}
		}
		...
		public int Runtime {
			get {
				...
				if (qstatInfo.TryGetValue(LinuxPbsJobInfoAttributes.RESOURCE_LIST_WALLTIME, out result)) {
					return LinuxPbsConversionUtils.ConvertQstatTimeStringToSeconds(result);
				}
				return 0;
			}
			set {
