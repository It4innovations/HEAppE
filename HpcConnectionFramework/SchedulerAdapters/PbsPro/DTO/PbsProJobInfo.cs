...
public class PbsProJobInfo : ISchedulerJobInfo
{
    ...
    public string SchedulerJobId
    {
        ...
        set
        {
            ...
            {
                IsJobArrayJob = true;
                SchedulerJobIdWoJobArrayIndex =
                    _schedulerJobId.Replace(match.Groups.GetValueOrDefault("IndexArray").Value, string.Empty);
    ...
    public string AllocatedNodesSplit
    {
        set
        {
            if (!string.IsNullOrEmpty(value))
                AllocatedNodes = Regex.Matches(value, @"(?<AllocationNode>[[a-z]+\d*)", RegexOptions.Compiled)
    ...
    ///     Job allocated nodes
    /// </summary>
    public IEnumerable<string> AllocatedNodes { get; private set; } = Enumerable.Empty<string>();
