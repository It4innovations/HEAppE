...
public class PbsProDataConvertor : SchedulerDataConvertor
{
    ...
    public override ClusterNodeUsage ReadQueueActualInformation(ClusterNodeType nodeType, object responseMessage)
    {
        ...
        var parameters = Regex.Matches(response, @"(?<Key>[^\s].*)( = |: )(?<Value>.*)", RegexOptions.Compiled)
            ...
            .Select(s => new
            {
                Key = s.Groups.GetValueOrDefault("Key").Value, s.Groups.GetValueOrDefault("Value").Value
            });
        ...
        FillingSchedulerJobResultObjectFromSchedulerAttribute(nodeType.Cluster, queueInfo,
            parameters.ToDictionary(i => i.Key, j => j.Value));
    ...
    public override SubmittedTaskInfo ConvertTaskToTaskInfo(ISchedulerJobInfo jobInfo)
    {
        ...
        return new SubmittedTaskInfo
        {
            ...
            AllocatedTime = Math.Round(
                obj.RunTime.TotalSeconds > 0
                    ? obj.RunTime.TotalSeconds
                    :
                    obj.StartTime.HasValue && obj.EndTime.HasValue
                        ?
                        (obj.EndTime.Value - obj.StartTime.Value).TotalSeconds
                        : 0, 3),
    ...
    public override IEnumerable<SubmittedTaskInfo> ReadParametersFromResponse(Cluster cluster, object responseMessage)
    {
        ...
        foreach (var jobResponseMessage in jobResponseMessages)
        {
            ...
            FillingSchedulerJobResultObjectFromSchedulerAttribute(cluster, schedulerResultObj,
                parameters.ToDictionary(i => i.Key, j => j.Value));
