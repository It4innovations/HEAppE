...
internal class SlurmJobAdapter : ISchedulerJobAdapter
{
    ...
    /// </summary>
    public object AllocationCmd => _jobCommandBuilder.ToString();
    ...
    public void SetNotifications(string mailAddress, bool? notifyOnStart, bool? notifyOnCompletion,
        bool? notifyOnFailure)
    {
                                                   ...
                                                   || (notifyOnCompletion ?? false) || (notifyOnFailure ?? false)))
        {
            ...
            if (notifyOnFailure ?? false) mailParameters += "FAIL,";

            if (notifyOnStart ?? false) mailParameters += "BEGIN,";

            if (notifyOnCompletion ?? false) mailParameters += "END,";
