...
public class LinuxLocalJobDTO : ISchedulerJobInfo
{
    ...
    public long Id
    {
        set => SchedulerJobId = value.ToString();
    }
    ...
    {
        set => AllocatedTime = TimeSpan.FromSeconds(value);
    }
    ...
    public Dictionary<string, string> AllParametres =>
        new()
        {
            ...
            { nameof(CreationTime), CreationTime == default ? string.Empty : CreationTime.ToString() },
            { nameof(SubmitTime), SubmitTime == default ? string.Empty : SubmitTime.ToString() },
