...
public class HyperQueueDataConvertor : SchedulerDataConvertor
{
    ...
    public override IEnumerable<string> GetJobIds(string responseMessage)
    {
        ...
        try
        {
            ...
            if (jsonObject.TryGetValue("id", out var idToken) && idToken.Type == JTokenType.String)
    ...
    public override SubmittedTaskInfo ConvertTaskToTaskInfo(ISchedulerJobInfo jobInfo)
    {
        ...
        if (hyperQueueJobInfo == null) throw new ArgumentException("JobInfo is not HyperQueueJobInfo");
        return new SubmittedTaskInfo
        {
            ...
            TaskAllocationNodes = jobInfo.AllocatedNodes?.Select(s => new SubmittedTaskAllocationNodeInfo
                    { AllocationNodeId = s, SubmittedTaskInfoId = long.Parse(jobInfo.Name) })
                .ToList(),
