...
public static class ObjectExtension
{
    ...
    public static int GetObjectHashCode(this object obj)
    {
        ...
        if (obj.GetType().GetInterfaces().FirstOrDefault(s => s.Name == "IEnumerable") != null)
        {
            ...
            if (objCollectionType == null) return hash;
    ...
    public static bool CompareRecursive(this object obj, object another)
    {
        ...
        foreach (var property in obj.GetType().GetProperties())
        {
            ...
            if (!property.PropertyType.IsClass || property.PropertyType == typeof(string))
            {
                ...
                {
                    if (objValue != anotherValue) return false;
                }
    ...
    private static bool CompareCollections(ICollection objCollection, ICollection anotherCollection)
    {
        ...
        if (objCollectionType == null || anotherCollectionType == null) return false;

        if (objCollectionType.GetType() != anotherCollectionType.GetType()) return false;

        if (objCollection.Count != anotherCollection.Count) return false;

        var objCollectionEnumerator = objCollection.GetEnumerator();
        var anotherCollectionEnumerator = anotherCollection.GetEnumerator();
        ...
        while (objCollectionEnumerator.MoveNext())
            if (anotherCollectionEnumerator.MoveNext())
            {
                if (!objCollectionType.IsClass || objCollectionType == typeof(string))
                {
                    ...
                    {
                        if (objCollectionEnumerator.Current != anotherCollectionEnumerator.Current) return false;
                    }
