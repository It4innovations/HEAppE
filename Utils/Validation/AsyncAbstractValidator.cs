...
///     Validator
/// </summary>
public abstract class AsyncAbstractValidator
{
    #region Constructor
    ...
    protected AsyncAbstractValidator(object validationObj)
    {
    ...
    protected static bool IsIpAddress(string text)
    {
        return Regex.IsMatch(text,
                   ...
                   RegexOptions.Compiled) //ipv4
               || Regex.IsMatch(text, @"^(?:[A-F0-9]{1,4}:){7}[A-F0-9]{1,4}$", RegexOptions.Compiled) //ipv6 FULL
               || Regex.IsMatch(text,
                   @"^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$"); //hostname dns
    }
    ...
    protected static bool IsDomainName(string text)
    {
        ...
        return Regex.IsMatch(text,
            @"^(www\.){0,1}((?![0-9-])[A-Za-z0-9-]{1,63})((\.[A-Za-z]{2,33}))+(\/[A-Za-z0-9-]+)*(\/){0,1}$",
            RegexOptions.Compiled);
    }

    protected static bool IsIpAddressWithPort(string text)
    {
        var adressWithPort = text.Split(':');
        return adressWithPort.Length == 2 && IsIpAddress(adressWithPort[0]) &&
               int.TryParse(adressWithPort[1], out var x);
    }

    protected static bool IsDomainNamePort(string text)
    {
        var adressWithPort = text.Split(':');
        return adressWithPort.Length == 2 && IsDomainName(adressWithPort[0]) &&
               int.TryParse(adressWithPort[1], out var x);
    }
