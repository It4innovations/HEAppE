...
public class JobManagementService : IJobManagementService
{
    ...
    public async Task<SubmittedJobInfoExt> CurrentInfoForJob(long submittedJobInfoId, string sessionCode, bool forceDirectQuery = false)
    {
        ...
        try
        {
            ...
            using (var unitOfWork = UnitOfWorkFactory.GetUnitOfWorkFactory().CreateUnitOfWork(_logger))
            {
                ...
                bool isCallbackMode = IsCallbackModeEnabledForJob(unitOfWork, job);

                if (isCallbackMode)
                {
                    return await ExecuteCallbackCurrentInfoForJobAsync(unitOfWork, job, loggedUser, isAdmin, submittedJobInfoId, sessionCode, forceDirectQuery, cacheKey);
                }
                else
                {
    ...
    private static bool IsCallbackModeEnabledForJob(IUnitOfWork unitOfWork, SubmittedJobInfo job)
    {
        var cluster = job.Specification?.Cluster;
        if (cluster == null)
        {
            cluster = unitOfWork.SubmittedJobInfoRepository.GetQueryableWithoutFilters()
                .Include(j => j.Specification)
                    .ThenInclude(s => s.Cluster)
                .Where(j => j.Id == job.Id)
                .Select(j => j.Specification.Cluster)
                .FirstOrDefault();
        }
        ...
        return clusterConfig.EnableCallback;
    }

    private async Task<SubmittedJobInfoExt> ExecuteCallbackCurrentInfoForJobAsync(
        ...
        string sessionCode,
        bool forceDirectQuery,
        string cacheKey)
    {
        if (!forceDirectQuery)
        {
            // Callback mode default path: pull directly from DB (using cache if fresh)
            if (_cache.TryGetValue(cacheKey, out SubmittedJobInfoExt cachedJobInfo))
            {
                _logger.LogDebug("Returning cached job info in callback mode for job {JobId} for user {UserId}", submittedJobInfoId, loggedUser.Id);
        ...
        // Force direct system query requested by user
        bool isTerminalState = job.State is JobState.Finished or JobState.Failed or JobState.Canceled;
        bool allowSystemQuery = !isTerminalState || (isTerminalState && job.StateSource == JobStateSource.Callback);

        if (!allowSystemQuery)
        {
            _logger.LogInformation("Job {JobId} is in terminal state {State} with source {Source}. Skipping direct system query.", submittedJobInfoId, job.State, job.StateSource);
            var jobLogic = LogicFactory.GetLogicFactory().CreateJobManagementLogic(unitOfWork, _userOrgService, _sshCertificateAuthorityService, _httpContextKeys, _expirioService, _logger);
            var jobInfo = jobLogic.GetSubmittedJobInfoByIdForStatus(submittedJobInfoId, loggedUser, isAdmin);
            return jobInfo.ConvertIntToExt();
        }

        _logger.LogInformation("Executing forced direct system query for job {JobId} in callback mode (State: {State}, Source: {Source})", submittedJobInfoId, job.State, job.StateSource);

        try
        {
            // Execute physical scheduler query; GetActualTasksInfo updates DB and marks StateSource = UserVerified
            var directQueryResult = await GetActualTasksInfo(submittedJobInfoId, sessionCode);
            return directQueryResult;
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Direct system query for job {JobId} failed. Fallback to existing DB state.", submittedJobInfoId);
            var jobLogic = LogicFactory.GetLogicFactory().CreateJobManagementLogic(unitOfWork, _userOrgService, _sshCertificateAuthorityService, _httpContextKeys, _expirioService, _logger);
            var jobInfo = jobLogic.GetSubmittedJobInfoByIdForStatus(submittedJobInfoId, loggedUser, isAdmin);
            return jobInfo.ConvertIntToExt();
        }
    ...
    private async Task<SubmittedJobInfoExt> ExecuteLegacyCurrentInfoForJobAsync(
        IUnitOfWork unitOfWork,
        SubmittedJobInfo job,
        AdaptorUser loggedUser,
        bool isAdmin,
