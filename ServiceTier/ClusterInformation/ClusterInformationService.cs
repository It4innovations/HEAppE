...
public class ClusterInformationService : IClusterInformationService
{
    ...
    public ClusterInformationService(IMemoryCache cacheProvider, IUserOrgService userOrgService, ISshCertificateAuthorityService sshCertificateAuthorityService, IHttpContextKeys httpContextKeys, ILogger logger)
    {
    ...
    public ClusterClearCacheInfoExt ListAvailableClustersClearCache(string sessionCode)
    {
        ...
        var (loggedUser, projectIds) = UserAndLimitationManagementService
            .GetValidatedUserForSessionCode(sessionCode, unitOfWork, _userOrgService, _sshCertificateAuthorityService, _httpContextKeys, _logger, AdaptorUserRoleType.Administrator);
