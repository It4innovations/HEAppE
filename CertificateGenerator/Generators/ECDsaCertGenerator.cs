...
public class ECDsaCertGenerator : GenericCertGenerator
{
    ...
    ///     Curve
    /// </summary>
    private readonly ECCurve _curve = ECCurve.NamedCurves.nistP521;

    /// <summary>
    ///     Is allowed convert to PuTTY
    /// </summary>
    private readonly bool _isAllowedConvertToPuTTY;
    ...
    public override string ToPuTTYPublicKey()
    {
        ...
        var cipherType = $"ecdsa-sha2-nistp{_key.KeySize}";
        var sshrsaBytes = Encoding.Default.GetBytes(cipherType);
        var curveBytes = Encoding.Default.GetBytes($"nistp{_key.KeySize}");
