...
namespace Kerberos.NET.Client
{
    ...
    public class Krb5CredentialCache
    {
        ...
        internal void Add(TicketCacheEntry entry)
        {
            ...
            if (entry.Value is KerberosClientCacheEntry entryValue)
            {
                ...
                this.Credentials.Add(new Krb5Credential
                {
                    ...
                    Server = FromResponseS(entryValue.KdcResponse, entryValue.KdcResponse.Ticket.SName),
                    AuthData = new List<KrbAuthorizationData>(),
                    StartTime = entryValue.StartTime.Value,
        ...
        private static PrincipalName FromResponseS(KrbKdcRep asRep, KrbPrincipalName name)
        {
            return new PrincipalName(asRep.Ticket.SName.Type, asRep.Ticket.Realm, name.Name);
        }
