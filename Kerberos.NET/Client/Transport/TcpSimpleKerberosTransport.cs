...
{
    /// <summary>
    /// TcpKerberosTransport without pool and ScavengeWindow.
    /// </summary>
    public class TcpSimpleKerberosTransport : KerberosTransportBase
    {
        private const string TcpServiceTemplate = "_kerberos._tcp";
        private const string TcpServiceTemplatePasswd = "_kpasswd._tcp";

        private readonly ILogger<TcpSimpleKerberosTransport> logger;

        public TcpSimpleKerberosTransport(ILoggerFactory logger)
            : base(logger)
        {
            this.logger = logger.CreateLoggerSafe<TcpSimpleKerberosTransport>();
        ...
        )
        {
            try
            {
                using (var client = await this.GetClient(locatePreferredServer).ConfigureAwait(false))
                {
                    ...
                    var response = await ReadResponse(stream, cancellation, this.ReceiveTimeout).ConfigureAwait(false);
                    return response;
                }
            }
        ...
        private async Task<ITcpSocket> GetClient(Func<Task<Dns.DnsRecord>> locatePreferredServer)
        {
            ...
            do
            {
                ...
                try
                {
                    client = new TcpSimpleSocket();
                    connected = await client.Connect(target, this.ConnectTimeout).ConfigureAwait(false);
                }
