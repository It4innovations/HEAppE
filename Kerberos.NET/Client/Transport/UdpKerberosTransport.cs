...
namespace Kerberos.NET.Transport
{
    public class UdpKerberosTransport : KerberosTransportBase
    {
        ...
        )
        {
            ...
            try
            {
                return await SendMessage(encoded, target, cancellation, this.ReceiveTimeout);
            }
        ...
        private static async Task<byte[]> SendMessage(ReadOnlyMemory<byte> encoded, DnsRecord target, CancellationToken cancellation, TimeSpan receiveTimeout)
        {
            using (var client = new UdpClient(target.Target, target.Port))
            {
                ...
                // https://stackoverflow.com/questions/12638104/await-udpclient-receiveasync-with-timeout/78028222#78028222
                using (CancellationTokenSource ctSource = new())
                {
                    using (CancellationTokenSource linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(ctSource.Token, cancellation))
                    {
                        ctSource.CancelAfter(receiveTimeout);
                        var response = await client.ReceiveAsync(linkedTokenSource.Token).ConfigureAwait(false);
