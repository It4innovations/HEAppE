...
#pragma warning disable CA2000 // Dispose objects before losing scope
            if (this.Queue.TryPeek(out TcpSocket socket) && socket.LastRelease.Add(window) <= DateTimeOffset.UtcNow)
            {
                if (this.Queue.TryDequeue(out socket))
                {
                    ...
                    Console.WriteLine($"$$$$$$$$$$$$$$$----------------------> Poll        ref this.activeConnections: {this.activeConnections}");
                }
            }
...
#pragma warning restore CA2000 // Dispose objects before losing scope
            ...
            finally
            {
                ...
                {
                    Interlocked.Decrement(ref this.activeConnections);
                    Console.WriteLine($"##############----------------------> Interlocked.Decrement(ref this.activeConnections): {this.activeConnections}");
                }
            }
        ...
        public void Release(TcpSocket socket)
        {
            if (!socket.Connected)
            {
                ...
                Interlocked.Decrement(ref this.activeConnections);
                Console.WriteLine($"##############---------------------->Release(TcpSocket socket),      ref this.activeConnections: {this.activeConnections}");
        ...
        public void Dispose()
        {
            Console.WriteLine($"NAMED POOL DISPOSED !!!!!!!!!!!!!!!!11ººººººººººººººººººººººººººººººººººººººººººººººººººººººººº1");
