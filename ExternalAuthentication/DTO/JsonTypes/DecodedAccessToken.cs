...
public class DecodedAccessToken
{
    ...
    private ParsedJwtAttributes _parsedAttributes;
    ...
    private void ParsePairs(IEnumerable<string> rawPairs, List<ParsedJwtAttributes.OrganizationProjectPair> parsedPairs)
    {
        bool ParseOrganizationProjectPair(string rawPair, out ParsedJwtAttributes.OrganizationProjectPair parsedPair)
        {
            ...
            //"ORG=IT4I,PRJ=TEST1"
            if (!rawPair.StartsWith("ORG=")) return false;
            var delimiterIndex = rawPair.IndexOf(',');
            if (delimiterIndex == -1 || delimiterIndex == rawPair.Length - 1) return false;
    ...
    private void LoadAccessAttributes()
    {
        ...
        if (_parsedAttributes != null) return;
    ...
    /// <param name="project">Project name.</param>
    /// <returns>True if user can list project.</returns>
    public bool CanListProject(string project)
    {
        return CanListProject(project, KeycloakSettings.Organization);
    }
    ...
    /// <param name="project">Project name.</param>
    /// <returns>True if user can read project.</returns>
    public bool CanReadProject(string project)
    {
        return CanReadProject(project, KeycloakSettings.Organization);
    }
    ...
    /// <param name="project">Project name.</param>
    /// <returns>True if user can write project.</returns>
    public bool CanWriteProject(string project)
    {
        return CanWriteProject(project, KeycloakSettings.Organization);
    }
