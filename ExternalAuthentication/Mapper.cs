        ...
        try
        {
            ...
            foreach (var propertyInfo in obj.Attributes.GetType().GetProperties())
            {
                var jsonPropertyName = propertyInfo?.GetCustomAttribute<JsonPropertyAttribute>().PropertyName ??
                                       string.Empty;
                if (ExternalAuthConfiguration.RoleMapping.TryGetValue(jsonPropertyName, out var mappedRole))
                {
                    ...
                    foreach (var project in projects)
                        if (projectRoleMapping.TryGetValue(project.UUID, out var pp))
                        {
                            if (!pp.Roles.Contains(mappedRole)) pp.Roles.Add(mappedRole);
                        }
                        else
                        {
